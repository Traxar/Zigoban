...
const State = struct {
    ...
    fn destroy(self: *State, allocator: std.mem.Allocator) void {
        const ptr = self.reset(allocator);
...
pub const Level = struct {
    ...
    fn index(self: *Level, xy: @Vector(2, u8)) u16 {
        return xy[0] + self.sizeX * @as(u16, @intCast(xy[1]));
    }
    ...
    pub fn load(allocator: std.mem.Allocator, reader: *std.mem.TokenIterator(u8, .any)) !Level {
        ...
        const size = sizeX * @as(u16, @intCast(sizeY));
    ...
    pub fn render(self: *Level, renderer: *c.SDL_Renderer, width: u32, height: u32) void {
        ...
        while (xy[0] < self.sizeX) : (xy[0] += 1) {
            ...
            while (xy[1] < self.sizeY) : (xy[1] += 1) {
                const r = c.SDL_FRect{ .x = @floatFromInt(offsetX + size * xy[0]), .y = @floatFromInt(offsetY + size * xy[1]), .w = @floatFromInt(size), .h = @floatFromInt(size) };
                const color = self.at(xy).?.color();
                _ = c.SDL_SetRenderDrawColor(renderer, @truncate(color >> 24), @truncate(color >> 16), @truncate(color >> 8), @truncate(color));
